...
-- serial port class
io.Serial = {
    ports = {},     -- serial ports found in system
    ...
    del_us = os.del_us, -- microseconds delay function
    ...
    getPorts = function(self)
        ...
        if not os.getenv('ComSpec') then -- Posix
            pn = {'/dev/ttyS?', '/dev/ttyUSB?', '/dev/ttyBT?'}
        else -- Windows
            pn,pa = {'com?'}, 1
        end
    ...
    open = function(self, args)
        ...
        if type(args.port) == 'number' or tonumber(args.port) then args.port = io.Serial.ports[tonumber(args.port)] end
...
setmetatable(io.Serial, {
    __call = function(args)
        return io.Serial.open(io.Serial, args)
    end,
})
